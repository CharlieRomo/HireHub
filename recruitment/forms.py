...
class InternProfileForm(forms.ModelForm):
    # Campos personalizados para obtener la información del CV
    education = forms.CharField(widget=forms.Textarea, help_text="Describe tu formación académica.")
    work_experience = forms.CharField(widget=forms.Textarea, help_text="Describe tu experiencia laboral.")
    skills = forms.CharField(widget=forms.Textarea, help_text="Lista tus habilidades relevantes.")
    certifications = forms.CharField(widget=forms.Textarea, help_text="Menciona certificaciones o cursos que hayas completado.")
    languages = forms.CharField(widget=forms.Textarea, help_text="Idiomas que dominas y nivel de competencia.")
    
    class Meta:
        model = CustomUser  # Usamos el modelo de usuario personalizado
        fields = ['first_name', 'last_name', 'email', 'education', 'work_experience', 'skills', 'certifications', 'languages']  # Campos a completar
